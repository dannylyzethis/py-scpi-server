...
def register_measurement_commands(registry: CommandRegistry, state: PNAMeasurementSystem) -> None:
    ...
    def add(path, handler, *, query=False, parameters=(), available=None):
        registry.register(CommandSpec(tuple(path), handler, tuple(parameters), query=query, available=available))
    ...
    add((calc, HeaderNode("FORMat")),
        ...
        parameters=(ParameterSpec(ParameterType.ENUM, choices=DISPLAY_FORMATS),), available=channel_available)
    add((calc, HeaderNode("FORMat")),
    ...
    add((calc, HeaderNode("MATH"), HeaderNode("FUNCtion")),
        ...
        parameters=(ParameterSpec(ParameterType.ENUM, choices=MATH_FUNCTIONS),), available=channel_available)
    add((calc, HeaderNode("MATH"), HeaderNode("FUNCtion")),
    ...
    add((calc, marker), lambda inv, value: _marker_enable(state, inv, value),
        parameters=(boolean,), available=marker_available)
    add((calc, marker), lambda inv: _marker_query(state, inv, "enabled"),
        query=True, available=marker_available)
    ...
    add((calc, marker, HeaderNode("FORMat")), lambda inv, value: _marker_format(state, inv, value),
        parameters=(ParameterSpec(ParameterType.ENUM, choices=MARKER_FORMATS),), available=marker_available)
    ...
    add((calc, marker, HeaderNode("FUNCtion"), HeaderNode("EXECute")),
        ...
        parameters=(ParameterSpec(ParameterType.ENUM, choices=("MAX", "MIN")),), available=marker_available)
    ...
    add((display, window, HeaderNode("CATalog")),
        lambda inv: _trace_catalog(state, inv.indices["window"]), query=True, available=window_available)
    ...
    add((display, window, trace, HeaderNode("FEED")),
        ...
        query=True, available=display_available)
    ...
    add((display, window, trace, HeaderNode("STATe")),
        lambda inv, enabled: _trace_visible(state, inv, enabled), parameters=(boolean,), available=display_available)
    add((display, window, trace, HeaderNode("STATe")),
    ...
    add((display, window, trace, HeaderNode("TITLe"), HeaderNode("DATA")),
        lambda inv, value: _trace_title(state, inv, value), parameters=(string,), available=display_available)
    add((display, window, trace, HeaderNode("TITLe"), HeaderNode("DATA")),
        lambda inv: state.trace(inv.indices["window"], inv.indices["trace"]).title,
        query=True, available=display_available)
    add((display, window, HeaderNode("TRACe"), HeaderNode("NEXT")),
        lambda inv: str(_next_trace(state, inv.indices["window"])), query=True, available=window_available)
