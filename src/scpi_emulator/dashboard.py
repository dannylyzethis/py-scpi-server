    ...
    def log_command(self, instrument_name, command, response, error=None):
        ...
        with self.lock:
            ...
            if len(self.stats["last_minute_commands"]) > 1:
                ...
                self.stats["commands_per_minute"] = (
                    len(self.stats["last_minute_commands"]) / minutes_span
                )
    ...
    def get_stats(self):
        ...
        with self.lock:
            ...
            return {
                ...
                "uptime": round(uptime),
                "uptime_str": str(datetime.fromtimestamp(uptime) - datetime.fromtimestamp(0)).split(
                    "."
                )[0],
            }
            ...
            try:
                if instrument_id in self.manager.servers:
                    ...
                    if restarted:
                        ...
                        return jsonify(
                            {"status": "success", "message": f"Restarted {instrument_id}"}
                        )
                    else:
                        return jsonify(
                            {"status": "error", "message": f"Failed to restart {instrument_id}"}
                        ), 500
                else:
                    return jsonify(
                        {"status": "error", "message": f"Instrument {instrument_id} not found"}
                    ), 404
        ...
        def api_start_all():
            ...
            try:
                ...
                if started:
                    ...
                    return jsonify(
                        {"status": "error", "message": "Failed to start some servers"}
                    ), 500
            except Exception as e:
                return jsonify({"status": "error", "message": str(e)}), 500
