...
def vna(tmp_path, instrument_id="VNA-2PORT-EMU"):
    return SCPIInstrument(
        "Virtual VNA-2PORT-EMU", instrument_id, state_directory=tmp_path
    )
...
def test_named_state_file_round_trip_persists_existence_only(tmp_path):
    ...
    saved = json.loads((tmp_path / "VNA-2PORT-EMU" / "bench.sta").read_text())
...
def test_catalog_delete_and_instrument_isolation(tmp_path):
    first = vna(tmp_path, "one")
    second = vna(tmp_path, "two")
...
def test_missing_unsafe_and_malformed_files_report_errors_without_mutation(tmp_path):
    ...
    directory = tmp_path / "VNA-2PORT-EMU"
