...
def pna() -> SCPIInstrument:
    return SCPIInstrument("Virtual N5222B-EMU", "N5222B-EMU")
...
def test_frequency_forms_generate_one_coherent_measurement_axis() -> None:
    ...
    assert instrument.pna_measurements.selected(1).stimulus == (1e9, 2e9, 3e9)
    ...
    assert instrument.pna_measurements.selected(1).stimulus == (3e9, 4e9, 5e9)
...
def test_log_cw_and_power_sweeps_generate_the_expected_x_values() -> None:
    ...
    assert instrument.pna_measurements.selected(1).stimulus == pytest.approx(
        (1e9, 10**9.5, 1e10)
    )

    instrument.process_command("SENS:FREQ:CW 2.4GHz")
    instrument.process_command("SENS:SWE:TYPE CW")
    assert instrument.pna_measurements.selected(1).stimulus == (2.4e9,) * 3
    ...
    assert instrument.pna_measurements.selected(1).stimulus == (-20.0, -10.0, 0.0)
...
def test_model_frequency_and_port_limits_report_scpi_errors() -> None:
    instrument = pna()
    for command in ("SENS:FREQ:STOP 30GHz", "SOUR:POW3 -10", "SENS:BAND 0"):
...
def test_segment_lifecycle_builds_axis_and_segment_specific_timing() -> None:
    ...
    assert instrument.pna_measurements.selected(1).stimulus == (1e9, 2e9, 3e9, 4e9, 5e9)
