...
- [x] `scpi-201` — Pin the initial vna-2-port and vna-4-port models, firmware families, options, and application matrix.
...
5. VNA command manifest and capabilities: `scpi-201` → `scpi-202` → `scpi-203`
6. Base VNA engine: `scpi-301` → `scpi-302` → `scpi-303` → `scpi-304`
...
queue of readings that moves from nominal operation through drift, failure, and recovery. A VNA or
VNA application might return successive scalar results, complex traces, or gain-compression data.
